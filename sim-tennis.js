...
const TID = 6; // Tennis tournament ID
...
async function start() {
    ...
    if (events.error || events.length === 0) {
        console.log("종목이 없습니다. 시뮬레이션을 중단합니다.");
        return;
    }

    // Play prelim matches (Round Robin groups or similar)
    let iterations = 0;
    while (iterations < 5) {
        iterations++;
        ...
        for (const m of curPending) {
            ...
            // 6:4 세트 스코어 (테니스 룰 시뮬레이션)
            await fetchJson(`/api/tournaments/${TID}/matches/${m.id}/score`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ team1_set1: 6, team2_set1: 4, team1_set2: 0, team2_set2: 0, winner_team: m.team1_id, status: 'completed' })
            });
        }
    }
    ...
    while (iterations < 50) {
        ...
        for (const m of curPending) {
            ...
            await fetchJson(`/api/tournaments/${TID}/matches/${m.id}/score`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ team1_set1: 6, team2_set1: 3, winner_team: m.team1_id, status: 'completed' })
            });
            playedAny = true;
        }
    ...
    if (finalMatches.length === 0 || finalMatches.error) {
        console.log("경기가 하나도 없습니다.");
        return;
    }
    ...
    let printed = finalMatches.filter(m => m.round >= 900).map(m => {
        ...
        return `[${m.event_name}] ${isFinal ? '결승전' : 'R' + m.round} | ${m.team1_name || '?'} vs ${m.team2_name || '?'} | 스코어(세트) ${m.team1_set1}:${m.team2_set1} | 승자: ${actualWinnerName}`;
    });
    ...
    // Check if tournament actually had valid final structure
    if (evts.length === 0) {
        console.log("본선 경기가 생성되지 않았습니다.");

        // Let's print the group matches
        console.log("\n-- 예선 풀리그 현황 --");
        finalMatches.filter(m => m.round < 900).forEach(m => {
            console.log(`[예선] ${m.event_name} | ${m.team1_name || '?'} vs ${m.team2_name || '?'} | 승자: ${m.winner_team == m.team1_id ? m.team1_name : m.team2_name}`);
        });

    } else {
        console.log("\n모든 테니스 그룹 대진과 경기가 결승까지 정상적으로 생성 및 진행 완료되었습니다!");
    }
}
